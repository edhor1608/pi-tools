...
Claude-family model hints from supported providers are strict routing signals. A Pi spawn that explicitly requests, or would inherit, one is sent to the Claude backend instead. Rating-table keys map to executable Claude Code aliases as `fable-5` → `fable`, `opus-5` → `opus`, and `sonnet-5` → `sonnet`. Provider-qualified hints such as `openrouter/anthropic/claude-fable-5` become `claude-fable-5`; native aliases `fable`, `haiku`, `opus`, and `sonnet` route directly. Retired OpenCode references are rejected before Claude routing. An explicit Claude spawn with no model still preserves Claude Code's default rather than inheriting the parent model.

Paid OpenRouter root spawns require an explicit `openrouter/<model-id>`, persistent `allowPaidOpenrouter: true` in the user-owned `~/.pi/agent/pi-tools.json`, and `allowPaidOpenrouter: true` on the spawn. OpenRouter is excluded from bare-id resolution, inheritance, nested descendants, and fallback. OpenCode providers are retired and rejected explicitly. Other non-Claude Pi providers retain normal native routing.
