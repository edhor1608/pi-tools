...
Claude-family model hints from supported providers are strict routing signals. A Pi spawn that explicitly requests, or would inherit, one is sent to the Claude backend instead. Provider-qualified hints such as `openrouter/anthropic/claude-fable-5` become the Claude Code model name `claude-fable-5`; Claude Code aliases `fable`, `haiku`, `opus`, and `sonnet` route directly. Retired OpenCode references are rejected before Claude routing. An explicit Claude spawn with no model still preserves Claude Code's default rather than inheriting the parent model.
