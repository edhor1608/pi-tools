...
Preserve user and assistant conversation content. Represent tool history as a dual layer: full normalized tool activity remains visible in the Pi transcript through non-contextual custom entries, while compact per-turn summaries of relevant actions, changes, outputs, and failures participate in model context. Do not replay raw external protocol objects as Pi tool calls or flood context with routine reads, searches, and long command output. Retain provenance such as source harness, original session id and file, working directory, model, and import time as non-contextual Pi session metadata. Every import intentionally creates a new Pi session, even when the same external session was imported before; there is no deduplication, synchronization, or mutation of prior imports. Before importing, always ask which working directory the new Pi session should use rather than silently choosing the source session's directory or Pi's current directory. The source session remains unchanged.

### Rationale

A native Pi session integrates with Pi's existing resume, tree, compaction, naming, and model-switching behavior. The dual layer keeps the human-auditable transcript richer than the model context: useful tool outcomes remain available to the continuing agent without spending tokens on every mechanical operation. Normalized activity keeps the import valid across providers and changing external schemas, while provenance preserves traceability. This provides substantially more continuity than a summary-only handoff without the fragility of treating foreign files as writable Pi sessions.

### Consequences

Migration creates an immutable snapshot rather than a live two-way sync. Repeated imports intentionally create separate Pi sessions and may therefore appear as duplicates in `/resume`. Importing always includes an explicit target-working-directory step, adding one interaction even when the source and current directory match. Exact external UI events, hidden reasoning, permissions, and provider-specific tool semantics may not survive, although their useful visible output should. Import adapters and fixtures are required separately for Claude Code and Codex.
