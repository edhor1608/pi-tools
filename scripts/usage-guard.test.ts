...
import { decideUsageGuard, isCodexUsageCacheFresh, usageGuardThresholds, type UsageGuardLatches } from "../extensions/usage-guard.ts";
...
void test("usage guard crosses warning and wrap-up thresholds", () => {
	assert.equal(decideUsageGuard([window(79)], {}, thresholds, NOW).level, undefined);

	const warning = decideUsageGuard([window(80)], {}, thresholds, NOW);
	assert.equal(warning.level, "warn");
	assert.deepEqual(warning.windows, [window(80)]);

	const wrapup = decideUsageGuard([window(90)], warning.latches, thresholds, NOW + 1);
...
void test("usage guard latches each level once per window", () => {
	const first = decideUsageGuard([window(85)], {}, thresholds, NOW);
	const repeated = decideUsageGuard([window(86)], first.latches, thresholds, NOW + 1);
...
void test("usage guard re-arms after the usage window reset changes", () => {
	const first = decideUsageGuard([window(85, NOW + 60_000)], {}, thresholds, NOW);
	const nextWindow = decideUsageGuard([window(85, NOW + 120_000)], first.latches, thresholds, NOW + 60_001);
...
void test("usage guard prunes latches older than fourteen days", () => {
	...
	const decision = decideUsageGuard([], latches, thresholds, NOW);
