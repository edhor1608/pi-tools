...
const richState = (): ReviewLoopState => ({
	...
	reviewer: { backend: "claude", model: "claude-fable-5", subagentId: "sub-7" },
	findings: [{ id: "f1", title: "null deref", severity: "major", detail: "manager.ts:42" }],
	majorFindingHistory: [["major:off-by-one"]],
	// blockedReason/hardStopReason intentionally absent: the session file is a
	// JSON boundary, and JSON drops explicit-undefined keys.
});
...
void describe("round-trip", () => {
	...
	void test("serialization snapshots: later mutations do not leak into the persisted copy", () => {
		...
		state.majorFindingHistory[0]!.push("major:new");
		assert.equal(persisted.state.findings[0]!.title, "null deref");
		assert.deepEqual(persisted.state.majorFindingHistory, [["major:off-by-one"]]);
	});
});

void describe("in-flight reviews are non-durable", () => {
...
void describe("reading from session entries", () => {
	...
	void test("foreign and malformed entries are ignored", () => {
		...
		const branch: SessionEntryLike[] = [
			...
			entry(valid),
			entry({ version: 1, state: { phase: "not-a-phase" } }),
			entry("garbage"),
			{ type: "message" },
		];
	...
	void test("reading an in-flight snapshot from the branch yields a pending review", () => {
		const inFlight = serializeReviewLoopState({ ...richState(), phase: "reviewing", reviewInFlight: true });
