 ...
 *
 * Resume semantics: durable state (phase, round, fingerprint, findings,
 * reviewer info, blocked reason) survives a session resume, but an in-flight
 * reviewer PROCESS does not — `restoreReviewLoopState` therefore resets
 * `reviewInFlight` to false, which the FSM treats as "pending dispatch": the
 * next entry-gate pass re-dispatches the review against a fresh fingerprint.
 */
...
export const isPersistedReviewLoopState = (value: unknown): value is PersistedReviewLoopState =>
	isObject(value) && value.version === 1 && isReviewLoopState(value.state);

export function serializeReviewLoopState(state: ReviewLoopState): PersistedReviewLoopState {
	return { version: 1, state: cloneState(state) };
}

/**
 * Turn a persisted snapshot back into a runtime state. Reviewer processes are
 * non-durable: an in-flight review comes back as PENDING (`reviewInFlight:
 * false`, fingerprint cleared) so the entry gate re-dispatches it against the
 * tree as it exists after the resume — never trusting a stale in-flight run.
 */
