...
await test("the production timeout error names the tool and three-minute limit", () => {
	assert.equal(
...
await test("a hung tool call fails clearly and receives an abort signal", async () => {
...
await test("parent cancellation still stops the timeout wrapper immediately", async () => {
...
await test("the guard wraps each definition once and can discover later tools", () => {
	...
	const firstExecute = Reflect.get(first, "execute");
	guard.apply(registry);
	const firstWrappedExecute = Reflect.get(first, "execute");
	assert.notEqual(firstWrappedExecute, firstExecute);

	const second = createDefinition("second");
	const secondExecute = Reflect.get(second, "execute");
	definitions.set(second.name, second);
	guard.apply(registry);

	assert.equal(Reflect.get(first, "execute"), firstWrappedExecute);
	assert.notEqual(Reflect.get(second, "execute"), secondExecute);
});

await test("successful and terminating tool results pass through unchanged", async () => {
	const result = {
...
await test("the timeout is fresh for each tool call, not shared across calls", async () => {
	const execute = () =>
